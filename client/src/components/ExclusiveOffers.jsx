  ...
  return (
    <div className='flex flex-col items-center px-6 md:px-16 1g:px-24 x1:px-32 pt-20 pb-30'>
        <div className='flex flex-col md:flex-row items-center justify-between w-full' >
            ...
            <button className = 'group flex items-center gap-2 py-3 px-4 cursor-pointer mt-6 md:mt-0' > 
                View All Offers
                <img src = {assets.arrowIcon} alt = "arrow-right-icon" className = "h-4 group-hover:translate-x-1 transition-all"/>
            </button>
        </div>
          ...
          {exclusiveOffers.map((item)=> (
            <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage: `url(${item.image})`}}>
                ...
                <button className='flex items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>
                  View Offers
                  <img src = {assets.arrowIcon} alt = "arrow-icon" className = "h-4 group-hover:translate-x-1 transition-all"/>
                </button>
            </div>
          ))}
        </div>
